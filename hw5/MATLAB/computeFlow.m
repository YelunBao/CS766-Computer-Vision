function result = computeFlow(img1, img2, search_radius, template_radius, grid_MN)
    ...
    for i = 1:M
        for j = 1:N
            ...
            window_xs = max(1, row-search_radius);
            window_xe = min(H, row+search_radius);
            window_ys = max(1, col-search_radius);
            window_ye = min(W, col+search_radius);
            
            search_area = img2(window_xs:window_xe,window_ys:window_ye);
...
%             disp(size(corr_map));            
            ...
            U(i, j) = max_ind_col - template_radius ;
            V(i, j) = max_ind_row - search_radius + template_radius - 1;
        end
    end
