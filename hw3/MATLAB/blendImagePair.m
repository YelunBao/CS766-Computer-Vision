...
%BLENDIMAGEPAIR
    ...
    for c = 1:Cs
        ...
        elseif strcmp(mode, 'blend')
            ...
            % ADD YOUR CODE HERE
            masks = bwdist(~masks);
            norm_masks = masks ./ max(masks(:));
            
            maskd = bwdist(~maskd);
            morm_maskd = maskd ./ max(maskd(:));
            
            % normalized weighted blending
            channel_out = (im2double(S) .* norm_masks + im2double(D) .* morm_maskd) ./ (norm_masks + morm_maskd);
