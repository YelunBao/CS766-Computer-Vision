...
%               get you started.
    ...
    stitched_img(ref_start_y : ref_start_y + H_ref - 1,...
                 ...
                 :) = ref_img;
    % base_img = zeros(H_stitched, W_stitched, C_stitched);
    ...
    for n = 1:num_imgs
        ...
        [kp_stitched, kp_n] = genSIFTMatches(base_img, img_n);
        % ---------------------------------------
        % ADD YOUR CODE HERE
        % ---------------------------------------
        %pad_img_n = zeros(H_stitched, W_stitched, C_stitched);
        %pad_img_n(y_start : y_end-1, x_start : x_end-1, :) = result_img;
        [inliers_id, H_3x3] = runRANSAC(kp_n, kp_stitched, ransac_n, ransac_eps);
        %size(inliers_id)
        % after_img = showCorrespondence(img_n, stitched_img, kp_n(inliers_id, :), kp_stitched(inliers_id, :));
        % imwrite(after_img, 'after_ransac.png');

        % warped_dest_img = zeros(round(max(stitched_n_corners(:,2)) - round(min(stitched_n_corners(:,2)))), round(max(stitched_n_corners(:,1)) - round(min(stitched_n_corners(:,1)))), 3);
        %warped_dest_img_ht = size(warped_dest_img, 1);
        %warped_dest_img_wd = size(warped_dest_img, 2);

        [mask_n, result_img] = backwardWarpImg(img_n, inv(H_3x3), [W_stitched, H_stitched]);
        
        %pad_img_n = zeros(H_stitched, W_stitched, C_stitched);
        %pad_img_n(y_start : y_end-1, x_start : x_end-1, :) = result_img;
%         figure, imshow(result_img);
        %mask_n = rgb2gray(pad_img_n) ~= 0;

        % warped_dest_img = stitched_img;

        stitch_mask = rgb2gray(stitched_img) ~= 0;
        stitched_img = blendImagePair(result_img, mask_n, stitched_img, stitch_mask, 'blend'); 
        % figure, imshow(stitched_img);
